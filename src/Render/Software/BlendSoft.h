...
namespace SoftGL
{
                      ...
                      const BlendFactor &factor)
    {
        switch (factor)
        {
            ...
            case BlendFactor_ONE_MINUS_SRC_ALPHA:   return T(1.f) - T(srcAlpha);
            case BlendFactor_ONE_MINUS_DST_COLOR:   return T(1.f) - dst;
            case BlendFactor_ONE_MINUS_DST_ALPHA:   return T(1.f) - T(dstAlpha);
        }
        return T(0.f);
    }
    ...
    T calcBlendFunc(const T &src, const T &dst, const BlendFunction &func)
    {
        switch (func)
        {
            case BlendFunction_ADD:             return src + dst;
            case BlendFunction_SUBTRACT:        return src - dst;
            case BlendFunction_REVERSE_SUBTRACT:return dst - src;
            case BlendFunction_MIN:             return glm::min(src, dst);
            case BlendFunction_MAX:             return glm::max(src, dst);
        }
        return src + dst;
    }
