...
namespace SoftGL
{
    ...
    public: 
        ...
        void setImageData(const std::vector<std::shared_ptr<Buffer<RGBA>>> &buffers) override
        {
            if (multiSample)
            {
                LOGE("setImageData not support: multi sample textures");
                return;
            }
            ...
            GL_CHECK(glBindTexture(target_, texId_));
            GL_CHECK(glTexImage2D(target_, 0, glDesc_.internalformat, width, height, 0, glDesc_.format, glDesc_.type, buffers[0]->getRawDataPtr()));
    ...
    public:
        ...
        void setSamplerDesc(SamplerDesc &sampler) override
        {
            ...
            GL_CHECK(glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MIN_FILTER, OpenGL::cvtFilter(sampler.filterMin)));
            GL_CHECK(glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MAG_FILTER, OpenGL::cvtFilter(sampler.filterMag)));
            glm::vec4 borderColor = OpenGL::cvtBorderColor(sampler.borderColor);
        ...
        void setImageData(const std::vector<std::shared_ptr<Buffer<RGBA>>> &buffers) override
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                GL_CHECK(glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, glDesc_.internalformat, width, height, 0, glDesc_.format, glDesc_.type, buffers[i]->getRawDataPtr()));
            }
            if (useMipmaps)
            {
        ...
        void initImageData() override
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                GL_CHECK(glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, glDesc_.internalformat, width, height, 0, glDesc_.format, glDesc_.type, nullptr));
            }
            if (useMipmaps)
            {
                GL_CHECK(glGenerateMipmap(GL_TEXTURE_CUBE_MAP));
