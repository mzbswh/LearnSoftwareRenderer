    ...
    namespace View
    {
        ...
        public:
            ...
            void setupConfigPanelActions()
            {
                ...
                configPanel_->setResetRevverseZFunc([&]()->void
                {
                    waitRenderIdle();
            ...
            int drawFrame()
            {
                ...
                config_->triangleCnt = modelLoader_->getModelPrimitiveCnt();

                auto &viewer = viewers_[config_->rendererType];
                if (renderType_ != config_->rendererType)
                {
                    // change render type need to reset all model states
                    resetStates();
                    renderType_ = config_->rendererType;
                    viewer->create(width_, height_, outTexId_);
                }
                viewer->configRenderer();
                if (dumpFrame_)
                {
                    RenderDebugger::startFrameCapture(viewer->getDevicesPointer());
                }
                viewer->drawFrame(modelLoader_->getScene());
                if (dumpFrame_)
                {
                    dumpFrame_ = false;
                    RenderDebugger::endFrameCapture(viewer->getDevicesPointer());
                }
            ...
            {
                if (renderType_ != RENDER_TYPE_NONE)
                {
                    viewers_[renderType_]->waitRenderIdle();
                }
            }
        ...
        private:
            ...
            int renderType_ = RENDER_TYPE_NONE;
