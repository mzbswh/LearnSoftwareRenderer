    ...
    namespace View
    {
        ...
        bool Viewer::create(int width, int height, int outTexId)
        {
            ...
            if (!cameraDepth_)
            {
                ...
                cameraDepth_->setPerspective(glm::radians(CAMERA_FOV), (float)SHADOW_MAP_WIDTH / (float)SHADOW_MAP_HEIGHT, CAMERA_NEAR, CAMERA_FAR);
            }
            // create renderer
            if (!renderer_)
            {
            ...
            shadowPlacehoder_ = createTexture2DDefault(1, 1, TextureFormat_FLOAT32, TextureUsage_Sampler);
        ...
        void Viewer::drawShadowMap()
        {
            ...
            clearStates.clearDepth = config_.reverseZ ? 0.f : 1.f;
            renderer_->beginRenderPass(fboShadow_, clearStates);
            renderer_->setViewport(0, 0, SHADOW_MAP_WIDTH, SHADOW_MAP_HEIGHT);
            // set camera
            cameraDepth_->lookAt(config_.pointLightPosition, glm::vec3(0.f), glm::vec3(0.f, 1.f, 0.f));
            cameraDepth_->update();
        ...
        void Viewer::processFXAASetup()
        {
            ...
            if (!fxaaFilter_)
            {
                fxaaFilter_ = std::make_shared<QuadFilter>(width_, height_, renderer_, [&](ShaderProgram &program) -> bool
                {
        ...
        {
            pipelineSetup(skybox, skybox.material->shadingModel, {UniformBlock_Model}, [&](RenderStates &rs) -> void
            {
        ...
        {
            pipelineSetup(mesh, Shading_BaseColor, {UniformBlock_Model, UniformBlock_Scene, UniformBlock_Material}, [&](RenderStates &rs) -> void
            {
        ...
        void Viewer::setupModelNodes(ModelNode &node, bool wireframe)
        {
            ...
            {
                setupModelNodes(child, wireframe);
            }
        }
        ...
        void Viewer::drawScene(bool shadowPass)
        {
            ...
            updateUniformScene();
            updateUniformModel(glm::mat4(1.f), camera_->viewMatrix());
            // draw point light
            ...
            if (!shadowPass && config_.showSkybox)
            {
                ...
                updateUniformModel(glm::mat4(1.f), glm::mat3(camera_->viewMatrix()));
        ...
        void Viewer::drawModelNodes(ModelNode &node, bool shadowPass, glm::mat4 &transform, AlphaMode mode, float specular)
        {
            ...
            for (auto &child : node.children)
            {
                drawModelNodes(child, shadowPass, modelMatrix, mode, specular);
            }
        }
        ...
        void Viewer::drawModelMesh(ModelMesh &mesh, bool shadowPass, float specular)
        {
            ...
            {
                updateIBLTextures(mesh.material->materialObject.get());
            }
            // update shadow textures
            if (config_.shadowMap)
            {
                updateShadowTextures(mesh.material->materialObject.get(), shadowPass);
            }
            // draw mesh
            pipelineDraw(mesh);
        }

        void Viewer::pipelineSetup(ModelBase &model, ShadingModel shading, const std::set<int> &uniformBlocks, const std::function<void(RenderStates &rs)> &extraStates)
        {
            setupVertexArray(model);
            // reset materialObj if shadingModel changed
            if (model.material->materialObject != nullptr)
            {
                if (model.material->materialObject->shadingModel != shading)
                {
                    model.material->materialObject = nullptr;
                }
            }
        ...
        {
            auto &materialObj = model.material->materialObject;
        ...
        void Viewer::setupMainBuffers()
        {
            ...
            if (!fboMain_->isValid())
            {
                LOGE("Failed to setupMainBuffers.");
            }
        }
        ...
        void Viewer::setupShadowMapBuffers()
        {
            ...
            if (!texDepthShadow_)
            {
                ...
                {
                    LOGE("Failed to setupShadowMapBuffers.");
                }
            }
        }
        ...
        void Viewer::setupTextures(Material &material)
        {
            ...
            {
                material.textures[MaterialTexType_SHADOWMAP] = shadowPlacehoder_;
            }
        ...
        void Viewer::setupSamplerUniforms(Material &material)
        {
            for (auto &kv : material.textures)
            {
                ...
                if (samplerName)
                {
                    ...
                    material.materialObject->shaderResources->samplers[kv.first] = std::move(uniform);
                }
            }
        }
        ...
        bool Viewer::setupShaderProgram(Material &material, ShadingModel shading)
        {
            ...
            {
                material.materialObject->shaderProgram = cachedProgram->second;
                material.materialObject->shaderResources = std::make_shared<ShaderResources>();
                return true;
            }
            ...
            if (success)
            {
                ...
                material.materialObject->shaderProgram = program;
                material.materialObject->shaderResources = std::make_shared<ShaderResources>();
            } 
            else
            {
        ...
        void Viewer::setupPipelineStates(ModelBase &model, const std::function<void(RenderStates &rs)> &extraStates)
        {
            ...
            {
                material.materialObject->pipelineStates = it->second;
            }
            else
            {
                auto pipelineStates = renderer_->createPipelineStates(rs);
                material.materialObject->pipelineStates = pipelineStates;
                pipelineCache_[cachekey] = pipelineStates;
            }
        }

        void Viewer::setupMaterial(ModelBase &model, ShadingModel shading, const std::set<int> &uniformBlocks, const std::function<void(RenderStates &rs)> &extraStates)
        {
            auto &material = *model.material;
            ...
            {
                material.materialObject = std::make_shared<MaterialObject>();
                material.materialObject->shadingModel = shading;
                ...
                for (auto &key : uniformBlocks)
                {
                    ...
                    if (uniform)
                    {
                        material.materialObject->shaderResources->blocks[key] = uniform;
                    }
                }
            }
        ...
        bool Viewer::initSkyboxIBL()
        {
            ...
            // convert equirectangular to cubemap if needed
            ...
            if (texCubeIt == skybox.material->textures.end())
            {
                ...
                if (texEqIt != skybox.material->textures.end())
                {
                    ...
                    LOGD("Convert equirectangular to cubemap: %s", success ? "success" : "failed");
                    if (success)
                    {
                        textureCube = texCvt;
                        ...
                        skybox.material->materialObject = nullptr;
                    }
                }
            }
            else
            ...
            if (!textureCube)
            {
                LOGE("Failed to initSkyboxIBL. skybox texture cube not available.");
                return false;
            }
            ...
            {
                LOGE("Failed to generate irradiance map.");
                return false;
            }
            ...
            auto texPrefilter = createTextureCubeDefault(kPrefilterMapSize, kPrefilterMapSize, TextureUsage_AttachmentColor | TextureUsage_Sampler);
            ...
            {
                LOGE("Failed to generate prefilter map.");
                return false;
            }
        ...
        void Viewer::updateIBLTextures(MaterialObject *materialObject)
        {
            if (!materialObject->shaderResources)
            {
                return;
            }

            auto &samplers = materialObject->shaderResources->samplers;
            if (iBLEnabled())
            {
        ...
        void Viewer::updateShadowTextures(MaterialObject *materialObject, bool shadowPass)
        {
            if (!materialObject->shaderResources)
            {
                return;
            ...
            if (shadowPass)
            {
                samplers[MaterialTexType_SHADOWMAP]->setTexture(shadowPlacehoder_);
            }
            else
            {
        ...
        size_t Viewer::getPipelineCacheKey(Material &material, const RenderStates &rs)
        {
            ...
            HashUtils::hashCombine(seed, (int)material.materialObject->shadingModel);
            // TODO pack together
        ...
        bool Viewer::checkMeshFrustumCull(ModelMesh &mesh, const glm::mat4 &modelMatrix)
        {
            BoundingBox bbox = mesh.aabb.transform(modelMatrix);
