...
namespace SoftGL
{
    namespace View
    {
        QuadFilter::QuadFilter(int width, int height, const std::shared_ptr<Renderer> &renderer, const std::function<bool(ShaderProgram &program)> &shaderFunc)
        {
            if (!renderer)
            {
                LOGE("QuadFilter: renderer is null");
                return;
            }
            width_ = width;
            height_ = height;
            ...
            quadMesh_.material->materialObject = std::make_shared<MaterialObject>();
            auto &materialObj = quadMesh_.material->materialObject;
            // renderer
            renderer_ = renderer;
            // fbo
            ...
            if (!success)
            {
                LOGE("QuadFilter: shaderFunc failed");
                return;
            }
        ...
        void QuadFilter::setTextures(std::shared_ptr<Texture> &texIn, std::shared_ptr<Texture> &texOut)
        {
            ...
            {
                LOGE("QuadFilter: input texture size mismatch");
                return;
            }
            if (width_ != texOut->width || height_ != texOut->height)
            {
                LOGE("QuadFilter: output texture size mismatch");
                return;
            }
            uniformFilter_.u_sceneSize = glm::vec2(width_, height_);
            uniformBlockFilter_->setData(&uniformFilter_, sizeof(UniformsQuadFilter));
